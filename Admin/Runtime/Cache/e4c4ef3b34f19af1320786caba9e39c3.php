...
<body>
	<div class="container">
		...
		<form class="form-inline" action="__URL__/dingdan" method="post">
        <div class="form-group">
            <input type="number" class="form-control" placeholder="请输入订单号" required="" name="dingdanhao">
        </div>
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo ($row['dingdanhao']); ?></td>
				<td><?php echo ($row['xiadanren']); ?></td>
				<td><?php echo ($row['zfff']); ?></td>
				<td><?php echo ($row['shff']); ?></td>
				<td><?php echo ($row['zt']); ?></td>
				<td><a href="__URL__/look/id/<?php echo ($row['id']); ?>" class="btn btn-info">详情</a></td>
				<td><a href="__URL__/edit/id/<?php echo ($row['id']); ?>" class="btn btn-primary">修改</a></td>
				<td><a href="__URL__/delete/id/<?php echo ($row['id']); ?>" class="btn btn-danger">删除</a></td>
			</tr><?php endforeach; endif; else: echo "" ;endif; ?>
	</table>
	<span class="pull-right"><?php echo ($show); ?></span>
